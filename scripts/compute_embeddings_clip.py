...
def makeClipClassesEmbeddings(data_file_path, classes):
	classes_embeddings = []

	model, _ = PretrainedModel.load_clip_model()
	...
	with open(data_file_path, 'w') as f:
		for c in tqdm(classes_embeddings, total=len(classes_embeddings)):
			text = c['text']
			emb = c['embedding']
			f.write(json.dumps({'text': text, 'embedding': emb.cpu().detach().numpy().tolist()}) + '\n')
...
# makeClipClassesEmbeddings('data/classes_clip.jsonl', classes)

def makeClipAugmentedDataset(new_data_file_path, old_data_file_path):
	if not os.path.exists(old_data_file_path):
		print('file not found:', old_data_file_path)
		return
	if os.path.exists(new_data_file_path):
	...
	clip_model, clip_transform = PretrainedModel.load_clip_model()
	...
	with open(new_data_file_path, 'w') as f:
		for line in tqdm(data, total=len(data)):
			print('#', end='')
			text = line['text']
			paraphrase = paraphrase_text(text, model=paraphraser_model, tokenizer=paraphrase_tokenizer, device=device, return_number=10)
			paraphrase += [text]
			text_embeddings = []
			for t in paraphrase:
